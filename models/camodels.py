...
from util.utils.tools import extract_image_patches, flow_to_image, \
    reduce_mean, reduce_sum, default_loader, same_padding


class Generator(nn.Module):
    def __init__(self, input_dim, output_dim, cnum):
        super(Generator, self).__init__()
        self.coarse_generator = CoarseGenerator(input_dim, self.cnum)
        self.fine_generator = FineGenerator(self.input_dim, output_dim, cnum)

    def forward(self, xin, x_stage1, mask):
        x_stage1 = self.coarse_generator(xin, mask)
...
class CoarseGenerator(nn.Module):
    def __init__(self, input_dim, output_dim, cnum):
        ...
        self.conv17 = gen_conv(cnum//2, output_dim, 3, 1, 1, activation='tanh')

    def forward(self, xin):
        # xin = torch.cat((x * m2, ones, m2))
...
class FineGenerator(nn.Module):
    def __init__(self, input_dim, output_dim, cnum):
        ...
        # 3 x 256 x 256
        self.conv1 = gen_conv(1, cnum, 5, 1, 2)
        ...
        # attention branch
        # 3 x 256 x 256
        self.pmconv1 = gen_conv(1, cnum, 5, 1, 2)
        ...
        self.pmconv6 = gen_conv(cnum*4, cnum*4, 3, 1, 1)
        ...
        self.allconv11 = gen_conv(cnum*8, cnum*4, 3, 1, 1, activation='relu')
        self.allconv12 = gen_conv(cnum*4, cnum*4, 3, 1, 1, activation='relu')
        if noskip:
            self.allconv13 = gen_conv(cnum*4, cnum*2, 3, 1, 1, activation='relu') #skip
        else:
            self.allconv13 = gen_conv(cnum*8, cnum*2, 3, 1, 1, activation='relu')
        self.allconv14 = gen_conv(cnum*2, cnum*2, 3, 1, 1, activation='relu')
        if noskip:
            self.allconv15 = gen_conv(cnum*2, cnum, 3, 1, 1, activation='relu') # skip
        else:
            self.allconv15 = gen_conv(cnum*4, cnum, 3, 1, 1, activation='relu')
        self.allconv16 = gen_conv(cnum, cnum//2, 3, 1, 1, activation='relu')
        self.allconv17 = gen_conv(cnum//2, output_dim, 3, 1, 1, activation='tanh')
# 
#     # def forward(self, xin, x_stage1, mask):
#     #     mask = mask.float()
#     #     x1_inpaint = x_stage1 * mask + xin * (1-mask)  # [b,1,256,256]
#     #     xnow = x1_inpaint
#     #     x = self.conv1(xnow)
#     #     x = self.conv2_downsample(x)
#     #     x = self.conv3(x)
...
# 
    def forward(self, xin, x_stage1, mask):
        ...
        # xnow = torch.cat([x1_inpaint, x_stage1, mask], dim=1)
        xnow = x_stage1
        ...
        x1_64_64_512 = torch.cat([pm, x_hallu], dim=1)
        x2_256_256_128 = torch.cat([xs_256_256_64, xa_256_256_64], dim=1)
        x3_128_128_256 = torch.cat([xs_128_128_128, xa_128_128_128], dim=1)

        # x = torch.cat([pm, pm], dim=1)
        # merge two branches
        x = self.allconv11(x1_64_64_512)
        x = self.allconv12(x)
        x = F.interpolate(x, scale_factor=2, mode='nearest')
        x_128_128_512 = torch.cat([x, x3_128_128_256], dim=1)
        x = self.allconv13(x_128_128_512)
        x = self.allconv14(x)
        x = F.interpolate(x, scale_factor=2, mode='nearest')
        x_256_256_256 = torch.cat([x, x2_256_256_128], dim=1)

        x = self.allconv15(x_256_256_256)
        x = self.allconv16(x)
        x = self.allconv17(x)
        x_stage2 = x
...
def gen_conv(input_dim, output_dim, kernel_size=3, stride=1, padding=0, rate=1,
             activation='relu', transpose=False):
    return Conv2dBlock(input_dim, output_dim, kernel_size, stride,
                       conv_padding=padding, dilation=rate,
                       activation=activation, norm='bn', transpose=transpose)
